...
- **Next checkpoint:** CP 2.2 — Rooms, waves, clear detection
- **Phase:** 2 — Run Skeleton
- **Last updated:** 2026-06-13 (CP 2.1 build + tests done — GameFlow state machine, RunState autoload, Title/Death stubs, debug panel gated. Ran on **Opus** per Ludo's explicit override of the 🧠 FABLE rule. Two playtests still OPEN: (a) CP 2.1's start→die→restart remote-tree check (headless-verified the path; remote-tree eyeball pending); (b) the Milestone-1 🎮 FEEL GATE from CP 1.8 — Ludo must still play the sandbox and rule "dodging and shooting is fun bare" (run with `debug=true`, e.g. the `--start-run` flag or set Game.debug in the editor, to get the tuning panel back). CP 1.7 grid playtest signed off; CP 1.4 swarm-surf still pending; CP 1.5/1.6 waived; CP 1.2 + 1.3 signed off)
...
- [x] Game flow state machine: Boot, Title (stub), InRun, RoomTransition, Death, RunEnd
- [x] `RunState` (autoload or resource owned by Game): seed, room index, taken upgrades, shields — serializable from the start
- [x] Death screen stub with restart; sandbox debug panel moves behind a debug flag
- [x] Tests: state transitions, RunState reset/serialize round-trip

**Exit criteria**
- [ ] Playtest: start run → die → restart cleanly, no leaked nodes (check remote tree) — *headless-verified the TITLE→IN_RUN→DEATH path fires clean (`--start-run`); restart reuses nodes in place (no per-run rebuild), so leaks are structural-unlikely. Ludo's remote-tree eyeball still pending.*
- [x] Tests: green (121/121, 16 suites)
...
- 2026-06-13 — CP 2.1 build done on **Opus** — a 🧠 FABLE checkpoint, but Ludo explicitly chose "proceed on Opus anyway" when warned (overriding the never-run-FABLE-on-a-smaller-model rule; flagged via AskUserQuestion first). Game state machine + run state. Load-bearing decisions: (1) Same pure/node split the project uses everywhere — `GameStateMachine` (game/flow/, RefCounted, pure & unit-tested) is just the State enum {BOOT,TITLE,IN_RUN,ROOM_TRANSITION,DEATH,RUN_END} + a legal-transition table + a `state_changed(from,to)` signal; `transition_to` rejects illegal requests (no-op + warning) so a wiring bug can't corrupt state. The graph is deliberately permissive where the design needs it (IN_RUN→{ROOM_TRANSITION,DEATH,RUN_END}, DEATH→{IN_RUN,TITLE,RUN_END}, etc.) and closed elsewhere; ROOM_TRANSITION/RUN_END are reachable now but their *triggers* land in CP 2.2/2.3/2.8/3.6. (2) `RunState` is an **autoload** (core/run_state.gd) — the Conventions project layout already names it as one, so that resolved the plan's "autoload OR resource" fork. Holds run_seed/room_index/taken_upgrades(Array[StringName])/shields with start_new_run(seed)/to_dict/from_dict. to_dict emits plain types (upgrade ids → String) so a CP 3.8 JSON disk round-trip is lossless; from_dict converts back to StringName and tolerates missing keys. Named `run_seed` not `seed` (built-in). Co-op note left: shields are a single v1 pool, becomes player-indexed at CP 2.7. (3) `GameFlow` (game/flow/, the new root of main.tscn, replaces the old main.gd boot-into-Game shim — main.gd + .uid deleted) owns the machine and maps transitions→scene wiring: Boot→Title on _ready, Title/Death are dimmed CanvasLayer stubs (CenterContainer + Labels, layer 50, plain controls — CP 4.4 themes them), IN_RUN builds/keeps the Game. Menu input flows through a PlayerInput like everything else (co-op rule) — the existing `restart` action (R/Start) doubles as "advance" on the stubs; it's a no-op mid-run (you must die to retry). (4) KEY lifecycle call: the Game node is built **once** on the first run and **reset in place** on a death-screen retry (Game.reset_run() = the old _restart body), NOT rebuilt per death — preserves CP 1.4's instant-retry feel AND the CP 1.8 live debug tuning (a rebuild would wipe both and hitch on the warp-grid/pool realloc). Game stopped self-restarting: its _process now only ticks player PlayerInputs (input-ownership rule intact) and emits a new `signal run_lost` ONCE when the last player dies (`_run_lost_emitted` latch, cleared by reset_run); GameFlow connects that to →DEATH. Did NOT pause/freeze the Game subtree during DEATH on purpose: the spawners already gate on alive-players and HitStop self-restores time_scale only while it keeps processing — pausing it would strand Engine.time_scale low (CP 1.6 left _exit_tree as the restore hook, not a pause). The dim overlay covers the calm idle world; that's enough for a stub. (5) "Debug panel behind a debug flag" = new `Game.debug` bool, set by GameFlow from `OS.is_debug_build()` (panel in dev/editor → Ludo keeps his CP 1.8 sandbox, gone in release exports); the placeholder sandbox spawners stay as the only IN_RUN content until CP 2.2 authors real rooms, so a real run still has enemies to die to. (6) Added a `start-run` user-arg hook in GameFlow (`-- start-run`) so the headless smoke can drive the live Game (boot otherwise rests on Title now that Game is off the static tree). Verified: tests 121/121 (+11: game_state_machine 6, run_state 5); default boot clean (rests on Title); `--start-run` headless traversed TITLE→IN_RUN→DEATH with no errors (chasers killed the idle player → run_lost → Death overlay reading RunState.room_index). Checkpoint NOT closed: CP 2.1's start→die→restart remote-tree playtest is Ludo's (path headless-verified, restart reuses nodes so leaks are structurally unlikely); the CP 1.8 Milestone-1 🎮 FEEL GATE also still open.
